...
Frontend changes are verified with a Playwright screenshot loop (see
`.agents/skills/visual-loop`): edit code, capture screenshots, view them,
compare against the spec, re-shoot until clean.

- `scripts/audit/shoot.mjs` — full-page screenshots of 10 page templates at
  desktop/tablet/mobile, plus interaction shots (nav dropdown, FAQ accordion,
  mobile menu); writes `manifest.json` with per-shot console/page errors:
...
- `scripts/audit/probe-styles.mjs` — extracts exact `getComputedStyle()` values
  (colors, font sizes, spacing, container widths) from any base URL into JSON,
  for numeric comparison against a design spec or the live site.
- Interactive browser control (click through states, inspect elements) is
  available via `npx playwright cli --browser=chromium` (system Chrome is not
  installed; use the bundled Chromium).

Playwright and its Chromium browser are already installed via
`devDependencies`. QA artifacts (`.launch/`, `.playwright-cli/`) are
gitignored.
