...
#    AFTER the new container is serving. Purging first (the old behavior)
#    let requests routed to the still-healthy old container repopulate the
#    cache with the previous release before traffic switched over. Purge
#    failures are non-fatal (stale edge cache heals on its own; the site
#    must still start).
...
if [ "$ready" -eq 0 ]; then
	echo '[entrypoint] WARN: server not ready after 60s; purging cache anyway' >&2
fi
